...
# *******************************  Datos Generales  ****************************************
    ...
    operador = fields.Selection([("ice", "ICE"), ("claro", "CLARO"),("Entreprise", "ENTERPRICE"), ("jasec", "JASEC"), ("otro", "OTRO"), ("Telefonica", "TELEFONICA")    ], 'Operadora', required=True)
    ...
    provincia = fields.Selection([('No Especificada', 'NO ESPECIFICADA'),('san_jose', 'San Jose'), ('alajuela', 'Alajuela'), ('cartago', 'Cartago'), ('heredia', 'Heredia'), ('guanacaste', 'Guanacaste'), ('puntarenas', 'Puntarenas'), ('limon', 'Limón')], string='Provincia:', required=True)
...
# *****************************   Datos del Cliente  *****************************************
    cliente = fields.Many2one('cliente.cliente', 'Cliente', required=True, ondelete='cascade')
    supervisor_cliente = fields.Many2one('res.partner', string='Supervisor:')
    jefe_proyecto_cliente = fields.Many2one('res.partner', string='Jefe Proyecto:')
    jefe_proyecto = fields.Many2one('res.partner', string='Jefe Proyecto:')
...
# ******************************  Datos  del Instalador*****************************************
    ...
    contrata_inst =fields.Many2one('res.partner', 'Contrata:', ondelete='cascade')
    jefe_cuadrilla_inst = fields.Many2one('res.partner', string='Jefe de Cuadrilla:')
    sup_telyman_inst = fields.Many2one('res.partner', string='Sup Telyman:')
...
# ******************************  Seguimiento  *****************************************
    seg_administrativo = fields.Many2one('tipo.seguimiento.administrativo', string='Seg Administrativo', track_visibility='onchange', index=True,
        default=lambda self: self.env.ref('telyman.seguimiento01').id, group_expand='_read_group_stage_ids',
        copy=False)

    seg_tecnico = fields.Many2one('tipo.seguimiento.tecnico', string='Seg Tecnico', track_visibility='onchange', index=True,
        default=lambda self: self.env.ref('telyman.seguimiento_tecnico01').id, group_expand='_read_group_stage_ids',
        copy=False)
...
# ******************************  Documentacion  *****************************************
    doc_estado = fields.Selection([('P', 'Pendiente'), ('E', 'Enviado'), ('R', 'Rechazar'), ('A', 'Aprobado')], string='Seg Tecnico:')
...
# ******************************  Check List  *****************************************
    ...
    estado_chck = fields.Selection([('No', 'No Establecido'), ('Po', 'Postpuesto'),('P', 'Programado'),  ('R', 'Rechazado'), ('A', 'Aceptado'), ('A', 'Enviado')], string='Estado:')
    ...
    items_relacionados_precio_total_aprobado=fields.Float('Total Items Aprobados', compute='_compute_total_Items', store=True)
    items_relacionados_precio_total_solicitado = fields.Float('Total Items Solicitados', compute='_compute_total_Items', store=True)
    ...
    cliente_orden_compra = fields.Many2one('cliente.cliente', 'Cliente Orden Compra', ondelete='cascade', store=False)
